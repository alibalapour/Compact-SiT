...
def get_args_parser():
    ...
    parser.add_argument('--reset-opt', action='store_true', default=False,
                        help='reset optimizer settings')
    ...
    # Dataset parameters
    parser.add_argument('--dataset', default='CIFAR10', choices=['CIFAR100', 'CIFAR10', 'STL10', 'TinyImageNet', 'UH_mini', 'UH_main', 'NCT', 'BreakHis_2', 'MHIST', 'Custom'],
    ...
    parser.add_argument('--custom_train_dataset_path', default='', type=str,
    ...
    parser.add_argument('--dataset-ratio', default=1, type=int, help='ratio of dataset for training')
    ...
    parser.add_argument('--validate-every', default=1, type=int,
                        help='validate and save the checkpoints every n epochs')
    ...
    parser.add_argument('--save-every-checkpoint', action='store_true', default=False,
                        help='save checkpoing model in every evaluation during training')
    ...
    # Sequence Pool / related to CCT model
    parser.add_argument('--seq-pool', action='store_true', default=False,
                        help='sequence pool uses all of output tokens for classification '
                             '(for more informateion check https://arxiv.org/abs/2104.05704)')
...
def main(args):
    # Initialize hardware setup for disributed training
    ...
    if args.training_mode == 'SSL':
        ...
        args.aa = ''  # set autoaugment policy to '' (Null)

        if args.SiT_LinearEvaluation == 1:
            print(
                "Warning: Linear Evaluation should be set to 0 during SSL training - changing SiT_LinearEvaluation to 0")
    ...
    if args.dataset_ratio:
        ...
        indices = random.sample(population=list(indices), k=int(dataset_len * args.dataset_ratio))
        dataset_train = torch.utils.data.Subset(dataset_train, indices)

    num_tasks = utils.get_world_size()  # world_size = number of distributed processes
    global_rank = utils.get_rank()  # rank = ???

    # ensures that different each augmented version of a sample will be visible to a different process (GPU)
    ...
    # mixup : mixup regularizes the neural network to favor simple linear behavior in-between training examples
    ...
    # Check finetune flag. If it's true, it loads model.
    if args.finetune:
    ...
    # Prepare model to run in parallel mode, if args.distributed is True
    ...
    # Set learning rate, optimizer, scaler, scheduler
    ...
    # Set loss function
    ###
    # SSL : MTL loss (Multi Task Loss)
    # finetune : soft target(label) cross entropy loss
    # other : cross entropy loss
    ###
    if args.training_mode == 'SSL':
    ...
    # Set checkpoint parameters to model, if args.resume is true
    if args.resume:
    ...
    # Evaluate pre-trained learned model
    if args.eval:
        test_stats = evaluate_SSL(data_loader_val, model, device, 0, args.output_dir)
        ...
        if epoch % args.validate_every == 0:
            if args.output_dir:
                if args.save_every_checkpoint:
                    checkpoint_path = 'checkpoint_epoch' + str(epoch) + '.pth'
                else:
                    checkpoint_path = 'checkpoint.pth'
                checkpoint_paths = [output_dir / checkpoint_path]
...
if __name__ == '__main__':
    ...
    Path('ouput').mkdir(parents=True, exist_ok=True)
