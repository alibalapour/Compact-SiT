...
def drop_rand_patches(X, X_rep=None, max_drop=0.3, max_block_sz=0.25, tolr=0.05):
    ...
    while total_pix < n_drop_pix:
        ...
        if X_rep is None:
            X[:, rnd_r:rnd_h, rnd_c:rnd_w] = torch.empty((C, rnd_h - rnd_r, rnd_w - rnd_c), dtype=X.dtype,
                                                         device='cuda').normal_()
        else:
...
def train_finetune(model: torch.nn.Module, criterion,
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.cuda.amp.autocast():
            ...
            print(torch.argmax(targets))
            loss = criterion(rot_p + contrastive_p, torch.argmax(targets))
...
def evaluate_SSL(data_loader, model, device, epoch, output_dir):
    ...
    for imgs1, rots1, imgs2, rots2 in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if i % print_freq == 0:
            ...
            print_out = './' + str(sample_counter) + '_' + str(i) + '.jpg'
            ...
            torchvision.utils.save_image(imagesToPrint, print_out, nrow=min(15, batch_size), normalize=True,
                                         range=(-1, 1))
...
def evaluate_finetune(data_loader, model, device):
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        # to avoid error when data has less than 5 classes
        try:
